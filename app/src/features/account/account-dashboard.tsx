...
export function AccountDashboard() {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <section className="border bg-card"><header className="border-b p-5"><p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Activity</p><h1 className="mt-1 text-xl font-semibold">Prediction history</h1></header>{data.predictions.length === 0 ? <div className="p-8 text-center"><p className="font-medium">No predictions yet</p><p className="mt-2 text-sm text-muted-foreground">Enter a live minute market to start building your record.</p><a className="mt-5 inline-flex min-h-10 items-center text-primary hover:underline focus-visible:outline-2 focus-visible:outline-ring" href="/">View live market</a></div> : <div className="divide-y">{data.predictions.map((item) => <article className="grid gap-3 p-4 sm:grid-cols-[auto_1fr_auto] sm:items-center sm:p-5" key={item.account}><div className="font-mono text-sm font-semibold">#{item.roundId ?? "—"}</div><div><p className="font-mono text-sm tabular-nums">Predicted {price.format(item.predictedPrice)}</p><p className="mt-1 text-xs text-muted-foreground">{item.actualPrice === null ? statusLabel(item.status) : `Closed ${price.format(item.actualPrice)}${item.error === null ? "" : ` · ${price.format(item.error)} away`}`}</p></div><div className="sm:text-right"><p className="font-mono text-sm tabular-nums">{item.claimType === "claim_refund" ? "Refund" : `${usdc.format(item.payoutUsdc)} USDC`}</p>{item.claimable ? <button aria-busy={busy === item.account} className="mt-2 min-h-10 border px-3 text-sm font-medium hover:bg-accent disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-ring" disabled={busy !== null} onClick={() => void runClaim(item)} type="button">{busy === item.account ? "Claiming…" : "Claim"}</button> : <p className="mt-1 text-xs text-muted-foreground">{item.claimed ? "Claimed" : statusLabel(item.status)}</p>}</div></article>)}</div>}</section>
