...
export function NoLiveMarket({ nowMs, rounds }: { nowMs: number; rounds: PublicRound[] }) {
  ...
  return (
    <main className="overflow-hidden border bg-card" aria-labelledby="empty-market-title">
      <section className="grid gap-8 border-b px-5 py-7 sm:px-8 sm:py-9 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-center">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl" id="empty-market-title">The next minute market is being prepared</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">The BTC oracle remains online. PulseCast will move into the next betting window automatically as soon as a round opens.</p>
          <div className="mt-6"><RefreshMarketsButton /></div>
        </div>
        <div className="border bg-background/50 p-5">
      ...
      <section aria-labelledby="recent-rounds-title">
        <div className="border-b px-5 py-4 sm:px-8"><p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Onchain history</p><h2 className="mt-1 font-semibold" id="recent-rounds-title">Recent rounds</h2></div>
        {recent.length === 0 ? (
...
function Cadence({ description, label, time }: { description: string; label: string; time: string }) {
  return <div className="border-b px-5 py-4 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0 sm:px-8"><div className="flex items-baseline justify-between gap-4"><p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">{label}</p><p className="font-mono text-sm font-semibold tabular-nums">{time}</p></div><p className="mt-2 text-sm text-muted-foreground">{description}</p></div>;
}
...
function RoundRow({ round }: { round: PublicRound }) {
  ...
  return (
    <article className="grid gap-4 px-5 py-4 text-sm sm:grid-cols-[0.8fr_1.2fr_auto] sm:items-center sm:px-8">
