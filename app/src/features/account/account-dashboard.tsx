    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <section className="border bg-card">
        ...
        {account.predictions.length === 0 ? (
          ...
          <div aria-label="Prediction history entries" className="max-h-96 divide-y overflow-y-auto overscroll-contain" role="region" tabIndex={0}>{account.predictions.map((item) => <PredictionRow busy={busy} item={item} key={item.account} onClaim={runClaim} />)}</div>
        )}
      </section>
